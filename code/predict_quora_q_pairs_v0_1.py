...
num_lstm = 255#np.random.randint(175, 275)
num_dense = 124#np.random.randint(100, 150)
rate_drop_lstm = 0.2#0.39#0.15 + np.random.rand() * 0.25
rate_drop_dense = 0.20#0.15 + np.random.rand() * 0.25
act = 'relu'#'relu'
...
x1 = Bidirectional(lstm_layer)(embedded_sequences_1)
# x1 = Bidirectional(lstm_layer)(x1)
x1 = Bidirectional(LSTM(num_lstm, return_sequences=False))(x1)

sequence_2_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedded_sequences_2 = embedding_layer(sequence_2_input)
# y1 = lstm_layer(embedded_sequences_2)
y1 = Bidirectional(lstm_layer)(embedded_sequences_2)
# y1 = Bidirectional(lstm_layer)(y1)
y1 = Bidirectional(LSTM(num_lstm, return_sequences=False))(y1)
...
early_stopping =EarlyStopping(monitor='val_loss', patience=5)#val_acc, monitor='val_loss', patience=3
...
#         class_weight=class_weight, callbacks=[early_stopping, model_checkpoint, history])

model.load_weights(bst_model_path)
bst_val_score = 0.2900# min(hist.history['val_loss'])
...
preds = model.predict([test_data_1, test_data_2], batch_size=2048, verbose=1) #batch_size=8192,
preds += model.predict([test_data_2, test_data_1], batch_size=2048, verbose=1) #batch_size=8192,
preds /= 2
