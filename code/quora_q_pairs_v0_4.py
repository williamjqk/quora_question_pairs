...
num_lstm = 255#255#np.random.randint(175, 275)
num_dense = 140#np.random.randint(100, 150)
rate_drop_lstm = 0.15#0.15 + np.random.rand() * 0.25
rate_drop_dense = 0.15#0.15 + np.random.rand() * 0.25
act = 'relu'#'relu'
...
lstm_layer_last = LSTM(num_lstm, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=False)

sequence_1_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedded_sequences_1 = embedding_layer(sequence_1_input)
# x1 = lstm_layer(embedded_sequences_1)
x1 = Bidirectional(lstm_layer1)(embedded_sequences_1)
# x1 = Bidirectional(lstm_layer)(x1)
...
# y1 = Bidirectional(lstm_layer)(y1)
...
# y1 = Dense(num_dense, activation=act)(y1)
...
class LossHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if TELE == True:
            ...
            guard.message("Loss {} , val_loss {}".format(logs.get('loss'),logs.get('val_loss')))
...
hist = model.fit([data_1_train, data_2_train], labels_train, \
        ...
        class_weight=class_weight, callbacks=[early_stopping, model_checkpoint, history]) # batch_size=2048,
